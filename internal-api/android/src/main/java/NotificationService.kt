...
/**
 * This service is responsible for checking if a new moment has been posted. If so, it will show a
 * notification to the user.
 *
 * Other notifications are ran in a periodic worker, see the [`NotificationWorker`] class for more
 * information.
 */
class NotificationService : Service() {
  ...
  private val runnableCode =
          object : Runnable {
            override fun run() {
              scope.launch { checkMoment() }
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      val notificationChannel =
              NotificationChannel(
                      ...
                      .apply { description = "Notifications for $channelName" }
